...
v0.4 使用独立的多对话索引。聊天界面选择“对话 / 新建”会打开对话列表，第一行“+ 新建对话”可直接建立空白对话；已有对话可切换、重命名和删除。新建对话不会清除其他历史，切换档案也不会再拿另一份历史覆盖当前对话。

上下文消息数决定一次请求最多携带多少条有效消息；实际活动消息、持久化消息和请求字符预算还受“资源限制”档位约束。待发送状态和错误响应不会作为正常上下文。“清空”只清除当前对话，不删除其他对话、档案 Key 或设置。
...
3. 填写模型档案，并按需配置联网搜索开关、预设、端点、搜索 API Key 和结果数；
...
`.j2cfg` 最大 32 KiB。外层和载荷均有版本标记，载荷使用 Base64 包装并带 CRC-32，用于发现传输损坏或意外修改。**Base64 和 CRC 都不是加密，也不能证明文件来自可信来源。** 手机端会先检查大小、格式、版本、Base64、载荷上限和 CRC，再接受档案与搜索设置。

导入档案配置不会导入聊天记录，也不会立即加载图片。配置包中出现的档案会替换对应槽位；**包中省略的档案会原样保留**，包括其 Key、端点、模型、开关和模型缓存，因此可以只导入一个自定义档案。包未指定活动档案时保留手机当前选择；明确指定时才切换。搜索设置同样是可选对象：旧 v2 包未携带它时保留当前值，新版离线网页与手机客户端导出的包会携带完整搜索设置。详细格式见 [离线生成器说明](../provisioner/README.md)。

## 8. 导出备份与恢复

导出会把四个档案、活动档案和搜索设置写成 `.j2cfg`，默认文件名为 `J2ME-LLM-backup-<毫秒时间戳>.j2cfg`，位置是手机报告的第一个可写文件系统根目录。每次导出使用新名称；若目标已存在，应用会拒绝写入，绝不会截断或覆盖旧备份。写入时先生成同目录临时文件，逐字节回读校验后再改成最终文件名。设备需要 JSR-75 文件写权限；部分手机会让用户选择存储卡或确认写入。
